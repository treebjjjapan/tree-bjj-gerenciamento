    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
         <div>
            ...
            <p className="text-slate-500 text-sm">Próximos alunos aptos para graduação baseados em frequência e tempo.</p>
         </div>
         <Award size={48} className="text-amber-500 opacity-20" />
      </div>

      <div className="space-y-4">
        {students.filter(s => s.attendanceCount > 20).map(s => (
          <div key={s.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex items-center space-x-8">
              ...
              <div className="flex-1 min-w-[120px]">
                <p className="text-[10px] font-black text-slate-400 uppercase mb-2">Progresso</p>
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                  <div className="bg-amber-500 h-full rounded-full" style={{width: `${Math.min((s.attendanceCount / 50) * 100, 100)}%`}}></div>
                </div>
              </div>
            </div>

            <button className="flex items-center space-x-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-emerald-600 transition-all">
              <Award size={16} />
              <span>Promover</span>
            </button>
          </div>
        ...
        <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl shadow-black/50 border border-white/20">
            ...
            <div className="w-32 h-32 bg-slate-900 rounded-full flex items-center justify-center mb-6 shadow-2xl shadow-black/30 border-4 border-white overflow-hidden p-2">
               <img 
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.parentElement!.innerHTML = '<div class="font-black text-2xl text-white">TREE</div>';
                }}
              />
            </div>
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full p-4 bg-slate-50 hover:bg-emerald-50 border border-slate-100 rounded-[2rem] flex items-center justify-between group transition-all"
            >
            ...
            <button 
              onClick={() => handleLogin(UserRole.PROFESSOR)}
              className="w-full p-4 bg-slate-50 hover:bg-blue-50 border border-slate-100 rounded-[2rem] flex items-center justify-between group transition-all"
            >
              <div className="flex items-center space-x-4 text-left">
            ...
            <button 
              ...
              className="w-full p-4 bg-slate-50 hover:bg-slate-200 border border-slate-100 rounded-[2rem] flex items-center justify-between group transition-all"
            >
              <div className="flex items-center space-x-4 text-left">
                <div className="w-12 h-12 bg-slate-300 rounded-2xl shadow-md flex items-center justify-center text-slate-700 font-black border border-slate-200">S</div>
                <div>
                  <p className="font-black text-slate-800 leading-none">Aluno</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Frequência e Loja</p>
                </div>
              </div>
